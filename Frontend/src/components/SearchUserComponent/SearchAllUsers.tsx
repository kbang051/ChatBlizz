...
const SearchAllUsers = () => {
    const { searchAllResults, handleFriendStatusToogle } = useUserSearchStore();
    
    // const [statuses, setStatuses] = useState(() =>
    //     searchAllResults.reduce((acc, user) => {
    //       acc[user.id] = user.status;
    //       return acc;
    //     }, {} as Record<string, string>)
    // );

    // const handleFriendRequestToogle = (userId: string) => {
    //     console.log("UsedId received: ", userId)
    //     console.log("Request received at handleFriendRequestToogle")
    //     console.log("Current statuses: ", statuses);
    //     console.log(statuses[userId]);
    //     setStatuses((prev) => ({
    //         ...prev,
    //         [userId]: prev[userId] === "unknown" ? "pending" : "unknown"
    //     }))
    // }
            ...
            {searchAllResults.map((item) => (
                ...
                <div>
                    <p className="text-sm text-gray-500">
                        ...
                        {item.status === "accepted" && <button className='bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-3 rounded cursor-pointer'> Friend </button>}
                    </p>
                </div>
                </div>
            ))}
